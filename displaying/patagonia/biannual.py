...
data = pm.rolling(time=2).mean()
data = data.dropna('time')

# get linear trend
data_trend = data.sel(time=slice('1951', '2021'))
...
plt.ylabel('2yr precipitation (mm/year)')
plt.xlim([1948, 2023])
plt.ylim([0, 600])
...
plt.savefig(join(currentdir, '../../../hyperdrought_data/png/PM_biannual.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
