...
# get linear trend
qn_trend = qn.sel(time=slice('1950', '2021'))
...
plt.plot(qn.sel(time=slice('1950','1979')).time.dt.year, qn.sel(time=slice('1950','1979')).time.dt.year*0+qn.sel(time=slice('1950','1979')).mean().values, lw=1.5, color='green', ls='--')
...
gammafit_66_15 = gamma.fit(qn.sel(time=slice('1950','1979')).values, floc=0, scale=1)
...
plt.axhline(qn.sel(time=slice('1950','1979')).mean(), color='green', lw=1.5, ls='--')
...
plt.savefig(join(currentdir,'../../../hyperdrought_data/png/PM_series_with_trend_RR_FET.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()
