...
# common period
pm_norm = pm.sel(time=slice('1950','2021'))/pm.sel(time=slice('1980','2020')).mean('time')*100
lens1_norm = lens1.sel(time=slice('1950','2021'))/lens1.sel(time=slice('1980','2020')).mean('time')*100
lens2_norm = lens2.sel(time=slice('1950','2021'))/lens2.sel(time=slice('1980','2020')).mean('time')*100
...
pm_1950_2021, ME_1950_2021 = obs_trend(pm_norm, '1950', '2021')
lens1_trends_1950_2021 = mod_trends(lens1, '1950', '2021')
lens2_trends_1950_2021 = mod_trends(lens2, '1950', '2021')

pm_1960_2016, ME_1960_2016 = obs_trend(pm_norm, '1960', '2016')
lens1_trends_1960_2016 = mod_trends(lens1, '1960', '2016')
lens2_trends_1960_2016 = mod_trends(lens2, '1960', '2016')

pm_1980_2021, ME_1980_2021 = obs_trend(pm_norm, '1980', '2021')
...
plt.sca(axs[0])
plt.errorbar(x=0.2, y=pm_1950_2021*10, yerr=ME_1950_2021*10, lw=1.2, color='r', capsize=10, fmt = 'o', capthick=1.5)
plt.boxplot(lens1_trends_1950_2021*10, notch=False, meanline=True,showmeans=True, positions=[0.8], patch_artist=True, showfliers=True, boxprops={'facecolor':'grey', 'alpha':0.6}, medianprops=dict(color='blue'), meanprops=dict(color='red'))
plt.boxplot(lens2_trends_1950_2021*10, notch=False, meanline=True,showmeans=True, positions=[1.4], patch_artist=True, showfliers=True, boxprops={'facecolor':'grey', 'alpha':0.6}, medianprops=dict(color='blue'), meanprops=dict(color='red'))
plt.ylim([-35,20])
plt.xlim([-0.1,1.7])
plt.ylabel('Norm. JFM precip at PM - 1950-2021 linear trend (%/dec)')
...
plt.sca(axs[1])
plt.errorbar(x=0.2, y=pm_1960_2016*10, yerr=ME_1960_2016*10, lw=1.2, color='r', capsize=10, fmt = 'o', capthick=1.5)
...
plt.ylim([-35,20])
plt.xlim([-0.1,1.7])
plt.ylabel('Norm. JFM precip at PM - 1960-2016 linear trend (%/dec)')
plt.xticks([0.2,0.8, 1.4], ["DMC (95% CI)","LENS1", "LENS2"], rotation=0)
plt.gca().spines.right.set_visible(False)
plt.gca().spines.top.set_visible(False)
plt.gca().tick_params(direction="in")
...
plt.sca(axs[2])
plt.errorbar(x=0.2, y=pm_1980_2021*10, yerr=ME_1980_2021*10, lw=1.2, color='r', capsize=10, fmt = 'o', capthick=1.5)
...
plt.ylim([-35,20])
plt.xlim([-0.1,1.7])
plt.ylabel('Norm. JFM precip at PM - 1980-2021 linear trend (%/dec)')
plt.xticks([0.2,0.8, 1.4], ["DMC (95% CI)","LENS1", "LENS2"], rotation=0)
plt.gca().spines.right.set_visible(False)
plt.gca().spines.top.set_visible(False)
plt.gca().tick_params(direction="in")
...
plt.tight_layout()
plt.savefig(join(currentdir, '../../../hyperdrought_data/png/PM_LENS1_LENS2_trend_validation.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()