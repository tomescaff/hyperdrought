...
# common period
pm = pm.sel(time=slice('1950','2021'))
lens1 = lens1.sel(time=slice('1950','2021'))
lens2 = lens2.sel(time=slice('1950','2021'))

# qn mu CI
mean = pm.mean()
std_dev = pm.std(ddof=1)
n = pm.size
dof = n-1
alpha = 0.05
p_star = 1-alpha/2
...
#plt.ylim([240, 960])
plt.xlim([-0.1,1.7])
plt.ylabel('JFM precip at PM - 1950-2021 mean value (mm)')
...
#plt.ylim([85,325])
plt.xlim([-0.1,1.7])
plt.ylabel('JFM precip at PM - 1950-2021 standard deviation (mm)')
plt.xticks([0.2,0.8, 1.4], ["DMC (95% CI)","LENS1", "LENS2"], rotation=0)
plt.gca().spines.right.set_visible(False)
plt.gca().spines.top.set_visible(False)
plt.gca().tick_params(direction="in")
...
#plt.ylim([-350,100])
plt.xlim([-0.1,1.7])
plt.ylabel('JFM precip at PM - 1950-2021 linear trend (mm/100yr)')
plt.xticks([0.2,0.8, 1.4], ["DMC (95% CI)","LENS1", "LENS2"], rotation=0)
plt.gca().spines.right.set_visible(False)
plt.gca().spines.top.set_visible(False)
plt.gca().tick_params(direction="in")
...
plt.tight_layout()
plt.savefig(join(currentdir, '../../../hyperdrought_data/png/PM_LENS1_LENS2_stats_validation.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()