...
qn = se.get_QN_annual_precip_long_record().sel(time=slice('1930','2021'))
cu = se.get_CU_annual_precip().sel(time=slice('1959','2021'))
ri = cr2met.get_regional_index().sel(time=slice('1960', '2021'))
...
se_norm = se/se.sel(time=slice('1991','2020')).mean('time')*100-100
...
plt.plot(se.sel(time='2019').time.dt.year, se.sel(time='2019').values, lw=1, marker='o', markersize=7, markeredgecolor='blue', markerfacecolor='blue', color='blue', alpha=0.4)
...
plt.tight_layout()
plt.savefig(join(currentdir, '../../../hyperdrought_data/png/QN_CU_RI_series_with_trend_full_from_1980.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()