...
annual_mean = annual.sel(time=slice('1991', '2020')).mean('time')
annual_std = annual.sel(time=slice('1991', '2020')).std('time')
annual_anom = (annual-annual_mean)/annual_std
da = annual_anom.sel(time='2019').squeeze()

mask = cr2met.get_cl_mask()
da = da*mask

stnfile = '../../../hyperdrought_data/CR2_explorer/cr2_pr_std_anom2019_refperiod_1991_2020_24deg_40degS.nc'
ds = xr.open_dataset(join(currentdir, stnfile))
...
pcm = ax.pcolormesh(da.lon.values, da.lat.values, da.values, cmap=cmaps.amwg_blueyellowred_r, zorder=4, vmin=-2, vmax=2)
cbar = plt.colorbar(pcm, aspect = 40, pad=0.03)

# plot the stations
ax.scatter(stn_lon, stn_lat, s=10, c=stn_val, cmap=cmaps.amwg_blueyellowred_r, zorder=5, vmin=-2, vmax=2, edgecolors='k')
...
plt.savefig(join(currentdir,'../../../hyperdrought_data/png/CR2METv25_central_chile_precip_1960_2021_std_anom_2019.png'), dpi=300, bbox_inches = 'tight', pad_inches = 0)
plt.show()