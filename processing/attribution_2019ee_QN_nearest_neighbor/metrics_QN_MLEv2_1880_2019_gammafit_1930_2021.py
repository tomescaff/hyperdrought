...
xopt = pmath.mle_gamma_2d_v2(qn_no2019.values, sm_no2019.values, [70, 4, -0.5])

sigma0, eta, alpha = xopt
sigma = sigma0*np.exp(alpha*smf/(eta*sigma0))
...
sig_ac_dist = bspreds_sigma0*np.exp(bspreds_alpha*Tac/(bspreds_sigma0*bspreds_eta))
sig_cf_dist = bspreds_sigma0*np.exp(bspreds_alpha*Tcf/(bspreds_sigma0*bspreds_eta))
...
df.to_csv(join(currentdir,f'../../../hyperdrought_data/output/metrics_QN_MLEv2_{cf_year}_{ac_year}_gammafit_1930_2021.csv'))